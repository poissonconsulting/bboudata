# Authors and Citation

## Authors

- **Troy Hegel**. Author. [](https://orcid.org/0000-0001-5363-7716)

- **Dave Hervieux**. Author.

- **Barry Nobert**. Author.

- **John Boulanger**. Author. [](https://orcid.org/0000-0001-8222-1445)

- **[Ayla
  Pearson](https://www.poissonconsulting.ca/author/ayla-pearson/)**.
  Author, maintainer. [](https://orcid.org/0000-0001-7388-1222)

- **[Joe
  Thorley](https://www.poissonconsulting.ca/author/joe-thorley/)**.
  Contributor. [](https://orcid.org/0000-0002-7683-4592)

- **[Seb
  Dalgarno](https://www.poissonconsulting.ca/author/seb-dalgarno/)**.
  Contributor. [](https://orcid.org/0000-0002-3658-4517)

- **Alan Constant**. Contributor.
  [](https://orcid.org/0000-0002-1652-4031)

- **Josie Hughes**. Contributor.  
  Multi-population and other extensions based on LandSciTech/bboudata

- **Province of Alberta**. Copyright holder.

- **His Majesty the King in Right of Canada, as represented by the
  Minister of the Environment**. Copyright holder.  
  Copyright applies to bbd_chk_data_survival(),
  bbd_chk_data_recruitment(), and associated helpers in R/functions.R,
  derived from LandSciTech/bboudata

## Citation

Source:
[`DESCRIPTION`](https://github.com/poissonconsulting/bboudata/blob/main/DESCRIPTION)

Hegel T, Hervieux D, Nobert B, Boulanger J, Pearson A (2026). *bboudata:
Data for bbou Project*. R package version 0.4.0,
<https://poissonconsulting.github.io/bboudata/>.

    @Manual{,
      title = {bboudata: Data for bbou Project},
      author = {Troy Hegel and Dave Hervieux and Barry Nobert and John Boulanger and Ayla Pearson},
      year = {2026},
      note = {R package version 0.4.0},
      url = {https://poissonconsulting.github.io/bboudata/},
    }
